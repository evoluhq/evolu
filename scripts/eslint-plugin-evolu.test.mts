...
describe("no-direct-task-call", { timeout: 10_000 }, () => {
  test("reports calling an intersection of Task", async () => {
    const result = await lintTypeScript(
      [
        'import type { Run, Task } from "../packages/common/src/Task.ts";',
  ...
  test("reports ordinary functions accepting an alias of Run", async () => {
    const result = await lintTypeScript(
      [
        'import type { Run } from "../packages/common/src/Task.ts";',
        "type AppRun = Run;",
  ...
  test("allows calling a Task factory", async () => {
    const result = await lintTypeScript(
      [
        'import type { Task } from "../packages/common/src/Task.ts";',
  ...
  test("reports calling a NextTask", async () => {
    const result = await lintTypeScript(
      [
        'import type { NextTask, Run } from "../packages/common/src/Task.ts";',
        "declare const run: Run;",
