...
const items = [1, 2, 3];
items.push(4);
items === items; // true, but content changed
...
Mutation also causes unintended side effects, makes code harder to predict, and complicates debugging. Use immutable values with readonly types.
