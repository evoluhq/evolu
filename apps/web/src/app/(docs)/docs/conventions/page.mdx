...
```ts
import { object, String, Number, type InferType } from "@evolu/common";
...
```ts
import { object, String, Number } from "@evolu/common";
...
- **No dynamic `this`** - Functions can be passed around as ordinary values
  without changing their behavior
- **Conciseness** - Callbacks, small functions, and currying need little syntax
- **Dependency injection** - Currying keeps dependencies explicit; see
...
method syntax. This matches implementations and avoids implicit method `this`
semantics when functions are passed as values.
