...
Use `Disposable` and `AsyncDisposable` when an object owns cleanup.

Prefer `DisposableStack` and `AsyncDisposableStack` over manual cleanup loops,
cleanup arrays, or `let disposed` / `isDisposed` flags.

Guard synchronous public methods with `assertNotDisposed` when use-after-dispose
is a programmer error.

For code using `move()`, prefer role-based naming:

- `using disposer = new DisposableStack()` while registering resources
- `const disposables = disposer.move()` for the transferred owner
