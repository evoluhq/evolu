...
### Subpath exports for specialized modules

When a module has many exports that would clash with common names or create verbose suffixed names, use [subpath exports](https://nodejs.org/api/packages.html#subpath-exports) instead:

```ts
// Instead of long suffixed names like takeSchedule, jitterSchedule, etc.
import { take, jitter, exponential } from "@evolu/common/schedule";
```

This pattern is appropriate when:

- The module is specialized (not needed by most users)
- Short names would clash with other exports
- Adding suffixes everywhere would hurt readability
...
```json
{
  "exports": {
    ...
    "./schedule": {
      "types": "./dist/src/schedule/index.d.ts",
      "import": "./dist/src/schedule/index.js"
    }
  },
  "typesVersions": {
    "*": {
      "schedule": ["./dist/src/schedule/index.d.ts"]
    }
  }
}
```

Use lowercase for subpath names (`schedule`, not `Schedule`) following Node.js conventions.
