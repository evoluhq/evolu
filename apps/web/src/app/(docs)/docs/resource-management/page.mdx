...
[TypeScript 5.2+](https://www.typescriptlang.org/docs/handbook/release-notes/typescript-5-2.html#using-declarations-and-explicit-resource-management) supports the `using` keyword. For runtime environments without native support (for example, Safari), Evolu provides [polyfills](/docs/api-reference/common/Polyfills).

Call `installPolyfills` early in your app:
