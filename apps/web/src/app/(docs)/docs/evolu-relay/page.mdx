...
Evolu Relay provides sync and backup for Evolu apps. Evolu apps can use multiple relays concurrently. For resilience, it's recommended to use two relays: a fast primary "home/company" relay (on‑prem or close to users) and a geographically distant secondary relay if the primary relay fails (hardware failure, network issues, etc.).
