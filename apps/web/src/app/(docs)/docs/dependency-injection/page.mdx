...
This guide explains [Pure DI](https://blog.ploeh.dk/2014/06/10/pure-di/) conventions Evolu uses for sync code. For async code, see [Task dependency injection](/docs/api-reference/common/Task/type-aliases/Task#dependency-injection).
...
Btw, Evolu provides [Console](/docs/api-reference/common/Console).

## Error Handling

Dependencies often perform operations that can fail. Use the [`Result`](/docs/api-reference/common/Result/type-aliases/Result) type to make errors explicit and type-safe. The key principle: **expose domain errors, not implementation errors**.
...
Evolu provides [`testCreateTime`](/docs/api-reference/common/Time/functions/testCreateTime) out of the box—a `Time` implementation that returns monotonically increasing values, useful for tests that need predictable, ordered timestamps.
