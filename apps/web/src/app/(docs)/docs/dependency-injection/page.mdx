...
Dependencies often perform operations that can fail. Use the [`Result`](/docs/api-reference/common/Result/type-aliases/Result) type to make errors explicit and type-safe. The key principle: **expose business errors, hide implementation errors**.

```ts
import { Result, ok, err } from "@evolu/common";

// Business errors that callers care about
...
// ✅ Good: Dependency interface exposes business errors
...
The implementation maps internal errors to business errors:
...
// ✅ Good: Map implementation errors to business errors
export const createStorage = (deps: SqliteDep): Storage => ({
  save: (data) => {
    const result = deps.sqlite.exec(/* ... */);
    if (!result.ok) {
      // Map SqliteError to a business error
...
This approach keeps your business logic decoupled from implementation details. You can swap SQLite for IndexedDB without changing the `Storage` interface or any code that depends on it.
...
Dependencies must not use generic type parameters—that would leak implementation details into business logic and tightly couple consumers to specific implementations.
