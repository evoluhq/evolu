...
- **Structured typed errors**: Keeps validation failures explicit and composable.
...
type CurrencyCode = typeof CurrencyCode.Output;
...
// Usage
const result = CurrencyCode.fromUnknown("USD");
if (result.ok) {
  console.log("Valid currency code:", result.value);
} else {
  console.error(CurrencyCode.formatError(result.error));
}
```
...
type SimplePassword = typeof SimplePassword.Output;
```
