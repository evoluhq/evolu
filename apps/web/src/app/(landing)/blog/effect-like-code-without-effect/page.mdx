...
<Note>
  **Update:** Since this article was published, the [Evolu
  Library](/docs/library) has grown significantly. It now provides
  [`Task`](/docs/api-reference/common/Task) for structured concurrency and
  resource management, with built-in dependency injection. The article still
  explains the ideas behind the library, but it no longer reflects its full
  capabilities.

We built our own standard library because we need to understand every line of
code and every design decision. We also have strict requirements for
correctness and our own, inevitably subjective, view of the best developer
experience, so we can’t—and don’t want to—build Evolu on Effect.
...
const result = NonEmptyString.fromUnknown("hello"); // Ok<NonEmptyString>
const error = NonEmptyString.fromUnknown(""); // Err<MinLengthError<1>>

// Many factories
export const Name = regex("Name", /^[a-z0-9-]{1,42}$/i)(NonEmptyString);

// string & Brand<"MinLength1"> & Brand<"Name">
